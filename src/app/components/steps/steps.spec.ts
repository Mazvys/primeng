...
import { Toast } from '@mazvysp/primeng/toast';
...
describe('Steps', () => {
    ...
    it('should readonly by default', () => {
        ...
        for (let x = 0; x < testComponent.items.length; x++) {
            if (x == 0) expect(items[x].nativeElement.className).not.toContain('p-disabled');
            else expect(items[x].nativeElement.className).toContain('p-disabled');
        }
    });
    ...
    it('should activate first item', () => {
        ...
        const items = fixture.debugElement.children[0].queryAll(By.css('li'));
        for (let x = 0; x < testComponent.items.length; x++) {
            if (x == 0) expect(items[x].nativeElement.className).toContain('p-highlight p-steps-current');
        }
    });
    ...
    it('should change activeItem', () => {
        ...
        const items = fixture.debugElement.children[0].queryAll(By.css('li'));
        for (let x = 0; x < testComponent.items.length; x++) {
            if (x == 2) expect(items[x].nativeElement.className).toContain('p-highlight p-steps-current');
        }
    });
